...
pf2 <- function (x, title = NA, counter = T, deseason = T, year_range = 2015:2021) {
  ...
  if (deseason) {
    with(sr[[x]]$des, {
      lines(x = time, y = fit, lty = 3, col = cols[1], lwd = 2)
    })}
  if (counter) {
    with(sr[[x]]$co2, {
      lines(x = time, y = fit, lty = 5, col = cols[2], lwd = 2)
    })}
  with(sr[[x]]$fit, {
    lines(x = nt[year %in% year_range, time], y = fit, col = cols[3], lwd = 2)
  })
  segments(covid_day, 0, y1 = ymax, lwd = 0.8, lty = 1)
...
tiff('Fig2_bw.tiff', height = 8, width = 10, units = 'in', res = 600, family = 'Tahoma')

par(xpd = NA, mar = c(3, 2, 3, 2), oma = c(2, 3, 0, 0))
pf2('irid')
mtext('Number of hospital admissions per day', side = 2, line = 1, outer = T)
text(covid_day, 42, '23 March 2020')
text(7000, 28, 'Counterfactual trend\nwith no slope or\nstep change', col = cols[2])
arrows(7000, 26, 7000, 21, length = 0.1, col = cols[2])
text(5500, 27.5, 'Trend with seasonality and\nslope/step change at\n23 March 2020', col = cols[3])
arrows(5500, 25, 5680, 19, length = 0.1, col = cols[3])
text(6200, 10, 'Deseasonalised\ntrend', col = cols[1])
arrows(6200, 11.5, 6050, 17.8, length = 0.1, col = cols[1])
